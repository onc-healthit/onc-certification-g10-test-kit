...
module ONCCertificationG10TestKit
  module Tasks
    class GenerateMatrix
      ...
      def test_procedure
        ...
        if File.file? File.join('lib', 'onc_certification_g10_test_kit', 'onc_program_procedure.yml')
          TestProcedure.new(
            YAML.load_file(File.join('lib', 'onc_certification_g10_test_kit', 'onc_program_procedure.yml')).deep_symbolize_keys
          )
        else
          TestProcedure.new
        end
    ...
    def generate_matrix_worksheet
      ...
      test_suite.groups.each do |group|
         ...
         group.groups.each do |test_case|
           ...
           cell = matrix_worksheet.add_cell(2, col, "#{test_case.short_id} #{test_case.short_title || test_case.title}")
    ...
    def generate_test_procedure_worksheet
    ...
    def generate_inferno_test_worksheet
      ...
      columns = [
        ['', 3, ->(_test) { '' }],
        ['', 3, ->(_test) { '' }],
        ['Inferno Test ID', 22, ->(test) { "#{test.short_id}" }],
