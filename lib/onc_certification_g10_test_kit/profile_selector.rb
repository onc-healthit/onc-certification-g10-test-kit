  ...
  module ProfileSelector
    ...
    def select_profile(resource) # rubocop:disable Metrics/CyclomaticComplexity
      ...
      when 'Observation'
        ...
        profiles << extract_profile('Bmi') if observation_contains_code?(resource, '39156-5')

        profiles << extract_profile('BloodPressure') if observation_contains_code?(resource, '85354-9')

        profiles << extract_profile('BodyHeight') if observation_contains_code?(resource, '8302-2')

        profiles << extract_profile('BodyTemperature') if observation_contains_code?(resource, '8310-5')

        profiles << extract_profile('BodyWeight') if observation_contains_code?(resource, '29463-7')

        profiles << extract_profile('HeartRate') if observation_contains_code?(resource, '8867-4')

        profiles << extract_profile('RespiratoryRate') if observation_contains_code?(resource, '9279-1')

        if (using_us_core_6? || using_us_core_7?) && observation_contains_code?(resource, '76690-7')
          profiles << extract_profile('ObservationSexualOrientation')
        end

        if (using_us_core_6? || using_us_core_7?) &&
           resource_contains_category?(resource, 'survey', 'http://terminology.hl7.org/CodeSystem/observation-category')
