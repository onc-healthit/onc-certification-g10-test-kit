...
module ONCCertificationG10TestKit
  class G10CertificationSuite < Inferno::TestSuite
    ...
    def self.jwks_json
      bulk_data_jwks = JSON.parse(File.read(File.join(__dir__, 'onc_certification_g10_test_kit',
                                                      'bulk_data_jwks.json')))
      @jwks_json ||= JSON.pretty_generate({ keys: bulk_data_jwks['keys'].select do |key|
                                                    key['key_ops']&.include?('verify')
                                                  end })
    end
